...
local SOCKET_CONNECT_TIMEOUT_TICK_TIME = 0.5 -- check socket connect timeout interval
...
function SocketTCP:setConnectTimeoutTickTime(__time)
	SOCKET_CONNECT_TIMEOUT_TICK_TIME = __time
	return self
end
...
function SocketTCP:connect(__host, __port, __retryConnectWhenFailure)
	...
	if not __checkConnect() then
		...
		local __connectTimeTick = function ()
			...
			self.waitConnect = self.waitConnect + SOCKET_CONNECT_TIMEOUT_TICK_TIME
		...
		self.connectTimeTickScheduler = scheduler.scheduleGlobal(__connectTimeTick, SOCKET_CONNECT_TIMEOUT_TICK_TIME)
	end
end
