...
local BugSpider = import("..model.BugSpider")
...
--        :addTo(self)
    local layout = require("layout.GameLayout").create()
    self:addChild(layout["root"])
    self.livesLabel_ = layout["root"]:getChildByName("livesLabel")
    self.bugsNode_ = display.newNode():addTo(layout["root"])
